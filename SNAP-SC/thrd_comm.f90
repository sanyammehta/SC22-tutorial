...
MODULE thrd_comm_module
  ...
  PUBLIC :: assign_thrd_set, destroy_task_set, no_op_lock_control,     &
    sweep_recv_bdry, sweep_send_bdry, sweep_recv_bdry_str, sweep_send_bdry_str

  PUBLIC :: sweep_bdry_t
!  PUBLIC :: sweep_recv_bdry_str
...
!_______________________________________________________________________
  ...
  type :: sweep_bdry_t
     INTEGER(i_knd) :: g, jd, kd, iop, t, nthrd, szreq, nc, &
          nang, ichunk, ny, nz
...
!___________________________________________________________________
     ...
     !INTEGER(i_knd) :: reqs
     !REAL(r_knd)    :: jb_out, jb_in
     !REAL(r_knd)    :: kb_out, kb_in
  end type sweep_bdry_t

!interface sweep_recv_bdry_str
!     module procedure sweep_recv_bdry
       !Import :: sweep_bdry_t
       !type(sweep_bdry_t) :: arg_struct
     !end subroutine sweep_recv_bdry_str
!end interface

CONTAINS

  SUBROUTINE sweep_recv_bdry_str (arg_struct, reqs, jb_in, kb_in)
     !Import :: sweep_bdry_t
     type(sweep_bdry_t) :: arg_struct

    INTEGER(i_knd), DIMENSION(arg_struct%szreq), INTENT(INOUT) :: reqs

    REAL(r_knd), DIMENSION(arg_struct%nang,arg_struct%ichunk,arg_struct%nz), INTENT(OUT) :: jb_in

    REAL(r_knd), DIMENSION(arg_struct%nang,arg_struct%ichunk,arg_struct%ny), INTENT(OUT) :: kb_in


     call sweep_recv_bdry ( &
          arg_struct%g,     &
          arg_struct%jd,    &
          arg_struct%kd,    &
          arg_struct%iop,   &
  ...
  SUBROUTINE sweep_send_bdry_str (arg_struct, reqs, jb_out, kb_out)
    ...
    REAL(r_knd), DIMENSION(arg_struct%nang,arg_struct%ichunk,arg_struct%nz), INTENT(OUT) :: jb_out

    REAL(r_knd), DIMENSION(arg_struct%nang,arg_struct%ichunk,arg_struct%ny), INTENT(OUT) :: kb_out


     call sweep_send_bdry ( &
          arg_struct%g,     &
          arg_struct%jd,    &
          arg_struct%kd,    &
          arg_struct%iop,   &
  ...
  END SUBROUTINE sweep_send_bdry_str
...
!_______________________________________________________________________
  ...
  SUBROUTINE sweep_recv_bdry ( g, jd, kd, iop, t, nthrd, reqs, szreq,  &
    nc, nang, ichunk, ny, nz, jb_in, kb_in )
!SUBROUTINE sweep_recv_bdry(arg_struct)
